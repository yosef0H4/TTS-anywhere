...
class DetectorSettings(BaseModel):
    # Reserved for future detector flags; currently model-only raw detection.
...
def create_app() -> FastAPI:
    app = FastAPI(title="Preprocessing RapidOCR Server", version="0.2.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
    ...
    def healthz() -> dict[str, object]:
        return {"ok": True, "detector": "rapidocr", "version": "0.2.0"}
    ...
    async def detect(image: UploadFile = File(...), settings: str | None = Form(default=None)) -> dict[str, object]:
        ...
        return {
            ...
            "settings": parsed_settings.model_dump(mode="json"),
            "raw_boxes": raw_boxes,
            "metrics": {
