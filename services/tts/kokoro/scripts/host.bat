...
if defined DRY_RUN (
  ...
  echo RUN="%UV_CMD%" run tts-kokoro serve --host %HOST% --port %PORT%
  exit /b 0
)
...
if "%UV_MODE%"=="global" (
  call uv run tts-kokoro serve --host %HOST% --port %PORT%
) else (
  call "%UV_CMD%" run tts-kokoro serve --host %HOST% --port %PORT%
)
