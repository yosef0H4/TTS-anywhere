...
```bat
py launcher.py --host 127.0.0.1 --port 8095
```
...
- the launcher itself can run under your existing system Python; it creates and validates the managed Python 3.11 GPU env
- that Windows tuple is chosen to match the available prebuilt Flash-Attention wheel set
- the launcher installs the torch-dependent Python stack separately, using the known-good `EyeHearYou` Mississippi setup (`transformers 4.57.3`, `accelerate 1.12.0`, `timm 1.0.24`, `peft 0.18.1`), so `uv sync` does not pull in a CPU-only torch
- the launcher manages a dedicated `.venv-gpu`
