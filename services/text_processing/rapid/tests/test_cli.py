...
def test_parse_args_with_feature_flags() -> None:
    args = cli.parse_args(["serve", "--enable-detect", "--enable-openai-ocr", "--execution-provider", "cpu"])

    assert args.cmd == "serve"
    assert args.enable_detect is True
    assert args.enable_openai_ocr is True
    assert args.execution_provider == "cpu"
...
def test_main_rejects_unavailable_explicit_gpu(monkeypatch: pytest.MonkeyPatch) -> None:
    original_parse_args = cli.parse_args
    monkeypatch.setattr(cli, "parse_args", lambda: original_parse_args(["serve", "--enable-detect", "--execution-provider", "cuda"]))
...
def test_main_starts_server_with_resolved_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    original_parse_args = cli.parse_args
    monkeypatch.setattr(cli, "parse_args", lambda: original_parse_args(["serve", "--enable-openai-ocr", "--execution-provider", "cpu"]))
    ...
    assert getattr(config, "execution_provider") == "cpu"
