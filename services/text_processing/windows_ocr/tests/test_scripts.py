...
def _run_script(script_name: str, env: dict[str, str], *args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["cmd.exe", "/c", str(SCRIPT_DIR / script_name), *args],
        cwd=SERVICE_ROOT,
        env=env,
        capture_output=True,
        text=True,
...
def test_batch_script_dry_run_uses_bundled_uv(tmp_path: Path) -> None:
    ...
    result = _run_script("host.bat", {**os.environ, "DRY_RUN": "1", "LOCALAPPDATA": str(tmp_path)}, "127.0.0.1", "8097", "en-US")

    assert result.returncode == 0
    assert f"BUNDLED_UV={bundled_uv}" in result.stdout
    assert f"UV_CMD={bundled_uv}" in result.stdout
    assert f"UV_PROJECT_ENVIRONMENT={SERVICE_ROOT}\\.venv" in result.stdout
